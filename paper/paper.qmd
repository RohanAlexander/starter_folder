...
pos_words <- get_sentiments("nrc") |>
  filter(sentiment = c("anger", "disgust", "fear", "negative", "sadness"))

#get negative words from NRC lexicon
neg_words <- get_sentiments("nrc") |>
  filter(sentiment = c("joy", "positive",
"surprise",
"trust", 
"anticipation"))
```

We then performed sentiment analysis on the free-response comment questions asked to students. To perform this text analysis, the tidytext package [@tidytext] was used in conjunction with the NRC word-emotion lexicon compiled by @mohammad. Although four lexicon options were available in tidytext, the NRC lexicon was chosen due to the relatively balanced amounts of positive and negative words in the dataset, allowing for balanced classification. 

...
We briefly describe the model that we use, which follows @kurztotherescue, which was adapted from @ospina. No students got zeroes, though some got full marks (100%) on the paper, so we adapt the regression to one-inflated beta model as described by @heisstotherescue to account for this.  

Model diagnostics are included in [Appendix -@sec-model-details].

The model uses a one-inflated beta regression approach to analyze the relationship between student LLM usage, GPA, and final paper marks. Let $Y_i$ represent the mark for student $i$, modeled as:
...
The model components are specified as:
...
The "llm_usage" predictor is categorical, with students' self-reported LLM usage classified as "Extensive" (reference level), "Somewhat", or "None or minimal". GPA is a continuous variable representing students' self-reported grade point averages. The model is implemented using the brms package in R, which provides a Bayesian framework for fitting these regression models.

The expected relationship between LLM usage and final mark is unclear, given the modelling strategy and variables collected. It could be that stronger students used LLMs in a more sophisticated way or that they did not need to use them. However, we can expect the relationship between GPA and final mark to be positive.

\newpage

# Results {#sec-results}

Our key results are summarized in @tbl-model-results and @fig-model-results, with full output available in the Appendix. We draw on the `modelsummary` [@modelsummary] package to summarize the results. 

@tbl-model-results and @fig-model-results present the key results from two models estimating the association between LLM usage, GPA and final paper mark. The first model includes only LLM usage, while the second model also includes self-reported GPA as a predictor.
...
The overall fit of the model improves when GPA is included. There is an solid increase in R-squared from 0.095 to 0.478 and improvements in the ELPD and LOOIC metrics. This is also suggested by the LOO comparison between the two models (@tbl-loo in [Appendix -@sec-model-details]).
...
Next, we use `bayesplot` [@bayesplot] and `loo` [@loo] conduct posterior predictive checks and evaluate model diagnostics.

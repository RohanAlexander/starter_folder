...
#| echo: false
#| warning: false
#| fig-cap: Distribution of grades on Final Paper
...
Transforming the regression with a logit function as the link function ensures that the predicted values stay between the 0-1 range, like in logistic regression. This allows the regression model to only return values between the 0-1, respecting the boundaries of the data. Finally, since students' grades could be either 0, 100, or in between, the zero-one-inflated beta model design is used to include grades that are 0 and 1 as well. 

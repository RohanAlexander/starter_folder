...
Trustworthy data science is the practice of conducting data analysis in a transparent, ethical, and reliable manner. These principles are reflected in various ways, through education, culture, and workflow.  To stay current, data scientists must continually update their knowledge of best practices for transparency and reproducibility, foster a culture which values different perspectives and accountability, and critically think about the broader impact of data-driven decisions.  The wide release of user-friendly Large Language Models (LLMs), particularly OpenAI’s ChatGPT, has transformed the data science toolkit, offering powerful tools for natural language processing and generation, thus enhancing aspects of data analysis and automation.

Like all new tools, however, LLMs have created both excitement and apprehension. ChatGPT's public release on November 30, 2022, brought LLMs into the mainstream conversation. LLMs have quickly been put to use in both industry and academia. By now, many people, especially educators and students, have some experience with LLMs, in both personal and professional contexts.

In the context of teaching statistics and data science, LLMs could be useful for many tasks. For instance, there is considerable interest in the potential of chatbots to act as personalized tutors for students [fulgencio2024developing; afzal2024tailoring; @Winkler_2018]. ChatGPT has also been the catalyst for many interesting and important conversations around academic integrity [@Eke2023], the development of critical thinking skills, and what effective learning looks like [@BaidooAnu_2023].
...
all_data |>
  ...
  mutate(
     ai_familiarity = factor(ai_familiarity, levels=c("Very familiar", "Somewhat familiar", "Not familiar"))
     ) |>
...
all_data |>
  ...
  mutate(
   ...
   mutate(
     ai_schoolwork_appropriate = factor(ai_schoolwork_appropriate, levels=c("Appropriate", "It depends", "Inappropriate"))
   ) |>
...
All classifications were then manually checked by hand for reasonableness.
...
all_data |>
  mutate(llm_usage = factor(llm_usage, levels = c("Extensive", "Somewhat", "Minimal", "None"))) |>
  count(llm_usage) |> 
...
all_data |>
  group_by(llm_usage) |>
  mutate(llm_usage = factor(llm_usage, levels = c(
    "Extensive", "Somewhat", "Minimal", "None"
  ))) |>

...
Just as @Cahill_2024 found, the vast majority of students taking the course were at least "somewhat familiar" with ChatGPT and other LLMs. However, this vast over-representation of familiarity could be driven by the fact that the course is primarily targeted for students in Statistics, Computer Science, Economics, Mathematics and other quantitative disciplines, leading to a sample that is skewed to being more interested in computing in general. 

Students were then asked about their attitudes and self-perception on writing and coding. First, students had varied attitudes toward their writing and their writing abilities in general, as seen in @fig-writing-multibar-1 and fig-writing-multibar-2.
...
multi_bar
```
We can see in @fig-writing-multibar-1 that most students believe it is important to be a good writing, though most are either indifferent or do not really like to write. In parallel way, students also do not find writing to be particularly easy, which could feed into the relative antipathy toward writing in general.  
...
```
Mirroring this, @fig-writing-multibar-2 shows that most students in the class were at least somewhat confident in their own writing abilities, but a significant contingent felt otherwise. Interestingly, although few students felt that they were confident writing ability, more felt that they were able to catch their mistakes, which could indicate a disconnect between how students perceive their work and how the work was actually evaluated. 

Students' self-perceptions about their coding abilities, shown in @fig-coding-multibar-1 and @fig-coding-multibar-2, also shed light on how students perceived themselves with respect to coursework, as R programming was a significant component of the course. 
...
Though students' self-perceptions around writing and coding are varied, there was strong consensus that the use of student AI tools is appropriate within an academic setting. One thing of note is that most who responded "It depends" generally found artificial intelligence tools to be appropriate, though with certain guidelines and rules governing their use.
...
### Task-Specific Attitudes 

Make a grid of all the tasks, with 

Students were also asked to rate the helpfulness of LLMs on various tasks assigned during the course on a 4 point scale of "Did not use" to "Very Helpful". To simplify the presentation, responses were grouped into two main categories: "Less Helpful" and "More Helpful." The "Less Helpful" category combines responses where students found the AI either "Not helpful" or did not use it for the task, while the "More Helpful" category includes responses where the AI was considered "Somewhat helpful" or "Very helpfu

```{r fig-AI-helpfulness-grid}
#| message: false
#| echo: false
#| warning: false
...
helpful_long <- helpful |>
  pivot_longer(cols = everything(), names_to = "task", values_to = "response")

# Simplify response categories and rename tasks
helpful_long <- helpful_long |>
  mutate(
    ...
    task = case_when(
      task == "ai_helpful_weekly_quiz" ~ "Weekly Quiz",
      task == "ai_helpful_weekly_mini_essay" ~ "Weekly Mini Essay",
      task == "ai_helpful_papers_generating_ideas" ~ "Generating Ideas",
      task == "ai_helpful_papers_writing_code" ~ "Writing Code",
      task == "ai_helpful_papers_writing_content" ~ "Writing Content",
      task == "ai_helpful_papers_improving_content" ~ "Improving Content",
      TRUE ~ "Other"  # Handle any unexpected task names
    )
  )

# Count the number of responses for each simplified category and task
helpful_summary <- helpful_long |>
  group_by(task, response_simplified) |>
  summarise(count = n(), .groups = 'drop')

custom_colors <- c(
  "Less Helpful" = "tomato",
  "More Helpful" = "lightgreen"
)

# Create grouped bar charts
grid_bar_chart <- helpful_summary |>
  ggplot(aes(x = response_simplified, y = count, fill = response_simplified)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ task, scales = "free_y") +
  labs(title = "Helpfulness of LLMs for Different Tasks", x = "", y = "Number of Students") +
  theme_minimal() +
  coord_flip() + 
...
grid_bar_chart <- addSmallLegend(grid_bar_chart, pointSize = 0.5, textSize = 9, spaceLegend = 0.5)

grid_bar_chart
```

@fig-AI-helpfulness-grid shows insights about how students used LLMs within the course. While most tasks were roughly split between students finding LLMs helpful or not, it was clear that students found them most helpful in generating code. In the context of the course, this meant generating R code for transforming, analyzing, and visualizing data. To a lesser extent, students also found LLMs to be helpful in improving the existing writing they had, while at the same time not favouring it for writing content from scratch. 
...
In the survey, students were asked to provide an open-ended response to the question: "Do you think using generative AI tools such as ChatGPT by OpenAI (or equivalents) is ethical and appropriate for schoolwork?". 

...
All the data was cleaned using R [@citeR] and it's tidyverse [@tidyverse], janitor [@janitor] and stringr [@stringr], then tested for issues using the testthat package [@testthat]. This led to a final dataset containing 121 responses across (var_num) variables. 
...
The student survey data was then visualized and analyzed using the ggplot2 [@ggplot2] package. 


### General Information

First, some general information about our sample of students was derived through visualizing the basic data about them. 
...
Programs of study [did something], as we can see in @fig-programs-pie... 
...
table_data |>
  ...
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y") +
  labs(title = "Student Familiarity with AI",
       x = NULL,
       y = NULL) +
  theme_minimal() +
  scale_fill_discrete(name="Program") +

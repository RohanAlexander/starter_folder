...
```{r}
# Import data (CHANGE WHEN FULLY CLEAN!)
...
raw_data |>
  ...
  labs(title = "Distribution of GPAs in the Survey Data",
       x = "GPA",
       y = "Frequency") +
  theme_minimal()
...
```{r fig-writingease-bar}
#Bar plot of writing ease
raw_data |> ggplot(aes(x = writing_is_easy_for_me)) +
  geom_bar(fill = "skyblue", color = "black") +
  labs(title = "Distribution of Responses to AI Schoolwork Appropriateness",
       x = "Response",
       y = "Frequency") +
  theme_minimal()  # <- fix the labels as they encroach. NO it broke.

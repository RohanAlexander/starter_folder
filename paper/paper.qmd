...
@Peng_2023 shows the potential impact of LLMs on students' computer programming skills, through their uncovering of the significantly positive impacts of GitHub Copilot (an LLM-powered programming assistant) on the productivity of computer programmers. In an experiment involving 95 freelance programmers, they found that that programmers with access to Copilot completed a standardized programming task 55.8% faster than the control group. Crucially, it was found that programmers with less experience saw the greatest improvements in productivity. Given that programming is a core component of data science practice, this suggests that these benefits would carry over to students in data science courses as well. 
...
The remainder of this paper is structured as follows: @sec-data visualizes and analyzes survey data collected from students at the end of the course; @sec-model models our unstructured data to approximate a relationship between grades and usage and attitudes; @sec-results lists the results; @sec-discussion discusses the implications of these findings for data science education and future research and practice in this rapidly evolving field.
...
ggplot(program_counts, aes(x = "", y = n, fill = program_of_study)) +
  ...
  coord_polar("y") +
  labs(title = "Student Distribution by Program of Study",
       x = NULL,
       y = NULL) +
  theme_minimal() +
...
```{r tbl-ai-familiarity}
#| message: false
#| echo: false
#| warning: false
#| tbl-cap: Familiarity with AI of students
#| fig-align: center
...
ggplot(appropriate, aes(x = "", y = n, fill = ai_schoolwork_appropriate)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Do students think AI use is appropriate in class?",
       x = NULL,
       y = NULL) +
  theme_minimal() +
